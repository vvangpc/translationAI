...
> 轻量自用 Chrome 翻译扩展 · 划词气泡 / 右键菜单 / 整页双语对照
...
市面上的 Chrome 翻译插件都太复杂 —— 词典、生词本、付费墙、十几个标签页的设置。这个扩展只做三件事：

1. **划词气泡翻译**：选中文字 → 弹出小猫图标 → 点击或 `Alt+T` → 气泡显示译文
2. **整页双语对照**：工具栏点击 / `Alt+A` → 每个段落下方追加译文，**不破坏页面布局**
3. **右键菜单翻译**：右键选中文字 → "翻译: ..." 一键触发
...
- 黑名单**只屏蔽划词图标自动弹出**，右键菜单 / 快捷键 / 整页翻译仍可用 —— 不打扰但不剥夺
...
- Shadow DOM（划词气泡样式隔离）
- TreeWalker + IntersectionObserver-style 视口排序（整页翻译）
